...
static uint32 hashtable_compute_hash(uint32 numBuckets, uint8 numBits,
...
uint32 hashtable_getnumentries(const struct hashtable *ht) { return ht->count; }
...
void hashtable_get_entry_idx(const struct hashtable *ht, size_t idx,
                             const void **key, size_t *keyLen,
                             void **clientData) {
  uint32 count;
  uint32 i;
...
static struct hashtable_entry *hashtable_lookup_entry(
    const struct hashtable *ht, uint32 hash, const void *key, size_t keyLen) {
...
bool hashtable_lookup(const struct hashtable *ht, const void *key,
                      size_t keyLen, void **clientData) {
  struct hashtable_entry *e;
  uint32 hash;
...
static void hashtable_check_resize(struct hashtable *ht) {
  struct hashtable_entry **buckets;
...
bool hashtable_insert(struct hashtable *ht, const void *key, size_t keyLen,
                      void *clientData) {
  struct hashtable_entry *e;
  uint32 hash;
...
static void hashtable_free_clientdata(const void *key, size_t keyLen,
                                      void *clientData) {
  free(clientData);
}
...
bool hashtable_remove(struct hashtable *ht, const void *key, size_t keyLen) {
...
void hashtable_insert_test(uint32 n, volatile int *stop) {
  ...
  Warning(LGPFX " stats: numEntries=%u maxDepth=%u emptyBuckets=%u\n",
          hashtable_getnumentries(ht), hashtable_getmaxdepth(ht),
...
static void hashtable_linearize_cb(const void *key, size_t keyLen, void *cbData,
                                   void *keyData) {
  struct hashtable_linearize_info *info =
      (struct hashtable_linearize_info *)cbData;

  memcpy((uint8 *)info->buf + info->idx * info->entry_size, keyData,
         info->entry_size);
  info->idx++;
}
...
void hashtable_linearize(const struct hashtable *ht, size_t entry_size,
                         void **ptr) {
  struct hashtable_linearize_info info;
  int n;

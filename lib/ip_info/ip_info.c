...
static void ipinfo_free_entry(const void *key, size_t keylen,
                              void *clientData) {
...
static void ipinfo_resolve_name_cb(void *clientData) {
  struct ipinfo_entry *entry = clientData;
  char host[256];
  uint32 count;
  int res;
  ...
  res = getnameinfo((struct sockaddr *)&entry->addr, sizeof(entry->addr), host,
                    sizeof host, NULL, 0, 0 /* flags */);
  if (res != 0) {
...
static size_t ipinfo_curl_write_cb(void *ptr, size_t size, size_t nmemb,
                                   void *userp) {
...
static void ipinfo_json_get(cJSON *root, const char *name, char **entry) {
  cJSON *item;
...
static void ipinfo_resolve_geo_cb(void *clientData) {
  struct ipinfo_entry *entry = clientData;
  struct buff *buf;
  char url[128];
  char *ipStr;
  ...
  Log(LGPFX " %s: %s, %s, %s, %s, %s\n", ipStr, entry->country_code,
      entry->country_name, entry->region_name, entry->city, entry->region_code);
exit:
  buff_free(buf);
  free(ipStr);
}

...
#define GET_ENTRY(_li) CIRCLIST_CONTAINER((_li), struct poll_entry, item);
...
static void poll_check_time_queue_order(const struct poll_loop *poll) {
...
static uint64 poll_get_device_key(int fd, bool read, bool write,
                                  bool permanent) {
...
static uint64 poll_get_device_key_from_entry(const struct poll_entry *e) {
...
static void poll_entry_dequeue(struct circlist_item **list,
...
static struct poll_entry *poll_entry_get(struct poll_loop *poll) {
...
static void poll_entry_put(struct poll_loop *poll, struct poll_entry *e) {
  ASSERT(poll);
  ASSERT(e);
...
static void poll_entry_ref(struct poll_entry *e) {
  ASSERT(e->refCount >= 0);
  e->refCount++;
}
...
static void poll_entry_unref(struct poll_loop *poll, struct poll_entry **in) {
...
static void poll_entry_remove_from_hashtable(struct poll_loop *poll,
                                             const struct poll_entry *e) {
  uint64 key;
  bool s;
...
static void poll_entry_free(struct poll_loop *poll, struct poll_entry *e) {
...
static void poll_free_entries_on_list(struct poll_loop *poll,
...
static void poll_insert_time(struct poll_loop *poll, struct poll_entry *entry) {
  static int count;
  ASSERT(poll && entry);
...
static int poll_get_max_fds(void) {
  struct rlimit lim;
  int res;
...
static void poll_device_poll(struct poll_loop *spoll, mtime_t deadline) {
...
static void poll_device_select(struct poll_loop *poll, mtime_t deadline) {
  struct circlist_item *li;
  int fds_max = -1;
  int s;
...
static mtime_t poll_get_next_expiry(const struct poll_loop *poll) {
  struct poll_entry *e;
...
static void poll_entry_fire(const struct poll_entry *e) {
  ASSERT(e);
  ASSERT(e->callback);
  ASSERT(e->queued);
  ASSERT(e->refCount > 0);
...
static bool poll_entry_active(const struct poll_loop *poll,
                              const struct poll_entry *e) {
  if (poll->use_poll) {
    int idx = e->u.d.idx;
...
static void poll_run_device_queue(struct poll_loop *poll) {
...
static struct poll_entry *poll_get_next_expired(struct poll_loop *poll,
                                                mtime_t now) {
  struct poll_entry *e;
...
static void poll_recalc_expiry(struct poll_entry *e) {
...
static void poll_run_time_queue(struct poll_loop *poll) {
  mtime_t now = time_get();
...
static void poll_dopoll_device(struct poll_loop *poll, mtime_t deadline) {
  if (poll->use_poll) {
...
void poll_runloop(struct poll_loop *poll, volatile int *exitPtr) {
  ASSERT(poll);
  ASSERT(exitPtr);
...
static void poll_insert_device(struct poll_loop *poll, struct poll_entry *e) {
  uint64 key;
  bool s;
...
void poll_callback_time(struct poll_loop *poll, mtime_t delayUsec,
                        bool permanent, pollcallback_fun *callback,
                        void *callbackData) {
  struct poll_entry *e;
  ...
  LOG(1,
      (LGPFX " %s: registering time CB fun=%p cbData=%p delay=%.1f msec p=%u\n",
...
void poll_callback_device(struct poll_loop *poll, int fd, bool readable,
                          bool writeable, bool permanent,
                          pollcallback_fun *callback, void *callbackData) {
  struct poll_entry *e;

  LOG(1, (LGPFX
          " %s: registering dev CB on fd=%d fun=%p cbData=%p r=%u w=%u p=%u\n",
          __FUNCTION__, fd, callback, callbackData, readable, writeable,
          permanent));
...
bool poll_callback_time_remove(struct poll_loop *poll, bool permanent,
                               pollcallback_fun callback, void *callbackData) {
  struct circlist_item **list;
  struct circlist_item *li;
  ...
  CIRCLIST_SCAN(li, *list) {
    ...
    if (e->callback == callback && e->callbackData == callbackData &&
        e->permanent == permanent) {
      e->queued = 0;
      poll_entry_dequeue(&poll->list_time, e);
      poll_entry_unref(poll, &e);
...
bool poll_callback_device_remove(struct poll_loop *poll, int fd, bool readable,
                                 bool writeable, bool permanent,
                                 pollcallback_fun callback,
                                 void *callbackData) {
  struct poll_entry *e;

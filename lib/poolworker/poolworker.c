...
#define GET_JOB(_li) CIRCLIST_CONTAINER(_li, struct poolworker_job, item);
...
static inline struct poolworker_job *poolworker_dequeue_job(
...
static inline void poolworker_destroy_job(struct poolworker_job *job) {
  ASSERT(job);
  ASSERT(job->func);
...
static inline void poolworker_execute_job(const struct poolworker_job *job) {
  ASSERT(job);
  ASSERT(job->func);
...
static void *poolworker_main(void *clientData) {
...
void poolworker_queue_work(struct poolworker_state *pw, poolworker_func *func,
                           void *clientData) {
  struct poolworker_job *job;

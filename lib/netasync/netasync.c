...
static const char *socks5_err2str(int e) {
  switch (e) {
    case 0:
      return "succeeded";
    case 1:
...
static const char *socks5_state2str(enum SocksState state) {
  switch (state) {
    case SOCKS_INIT:
      return "SOCKS_INIT";
    case SOCKS_CONNECTING_PROXY:
      return "SOCKS_CONNECTING_PROXY";
    case SOCKS_CONNECTED_PROXY_RECV:
      return "SOCKS_CONNECTED_PROXY_RECV";
    case SOCKS_CONNECTED_PROXY:
      return "SOCKS_CONNECTED_PROXY";
    case SOCKS_CONNECTED_REMOTE:
      return "SOCKS_CONNECTED_REMOTE";
    case SOCKS_CONNECTED_REMOTE_RECV:
      return "SOCKS_CONNECTED_REMOTE_RECV";
    case SOCKS_CONNECTED_OK:
      return "SOCKS_CONNECTED_OK";
  }
  return NULL;
}
...
static void netasync_timeout_stop(struct netasync_socket *sock) {
  bool s;
...
static void netasync_connect_stop(struct netasync_socket *sock) {
  bool s;

  LOG(2, (LGPFX " stop connect on %p -- %s\n", sock, sock->hostname));

  s = poll_callback_device_remove(netasync.poll, sock->fd, 0, /* read */
                                  1,                          /* write */
                                  0,                          /* permanent */
...
static void netasync_send_stop(struct netasync_socket *sock) {
  LOG(2, (LGPFX " stop send on %p -- %s\n", sock, sock->hostname));

  poll_callback_device_remove(netasync.poll, sock->fd, 0, /* read */
                              1,                          /* write */
                              0,                          /* permanent */
...
static void netasync_receive_stop(struct netasync_socket *sock) {
  LOG(2, (LGPFX " stop receiving on %p -- %s\n", sock, sock->hostname));

  poll_callback_device_remove(netasync.poll, sock->fd, 1 /* read */,
                              0 /* write */, 1 /* permanent */,
...
static void netasync_receive_reset(struct netasync_socket *sock) {
  sock->recvBuf = NULL;
  sock->recvBufLen = 0;
  sock->recvBufIdx = 0;
  sock->recvCb = NULL;
...
char *netasync_addr2str(const struct sockaddr_in *addr) {
  ...
  return safe_asprintf("%u.%u.%u.%u", (ip >> 24) & 0xFF, (ip >> 16) & 0xFF,
                       (ip >> 8) & 0xFF, ip & 0xFF);
}
...
static int netasync_socket_nonblock(const struct netasync_socket *sock) {
  int flags;
  int res;
...
static void netasync_accept_cb(void *clientData) {
...
int netasync_bind(struct netasync_socket *sock, const struct sockaddr_in *addr,
                  netasync_callback *cb, void *clientData) {
  int res;
  ...
  if (sock->fd < 0) {
    ...
    if (sock->err == EMFILE || sock->err == ENFILE) {
      Warning(LGPFX
              " Failed to create socket: EMNFILE: no more fd available.\n");
      Panic("EMNFILE.\n");
    } else {
  ...
  sock->bind = 1;
  poll_callback_device(netasync.poll, sock->fd, 1, /* read */
                       0,                          /* writable */
                       1,                          /* permanent */
...
static int netasync_getsocket_errno(const struct netasync_socket *sock) {
  socklen_t len;
  int err = 0;
  int res;
...
static void netasync_sock_recv_cb(struct netasync_socket *sock, void *recvBuf,
...
static void netasync_sock_send_cb(struct netasync_socket *sock,
...
static void netasync_socks_handler(struct netasync_socket *sock) {
  ...
  switch (sock->socks_state) {
    case SOCKS_CONNECTING_PROXY:
      ...
      break;
    case SOCKS_CONNECTED_PROXY_RECV:
      netasync_receive(sock, sock->socksRecvBuf, 2, 0, netasync_sock_recv_cb,
                       NULL);
      break;
    case SOCKS_CONNECTED_PROXY:
    ...
    case SOCKS_CONNECTED_REMOTE:
      // 10 = 6 + slen
      netasync_receive(sock, sock->socksRecvBuf, 10, 0, netasync_sock_recv_cb,
                       NULL);
      break;
    case SOCKS_CONNECTED_REMOTE_RECV:
      buf = sock->socksRecvBuf;
  ...
  if (sock->err == 0) {
    ...
    if (sock->err != 0) {
      Log(LGPFX " failed to connect fd=%d to %s (%s) - %s (%d)\n", sock->fd,
          sock->hostname, latStr, strerror(sock->err), sock->err);
    } else {
      Log(LGPFX " connected to %s fd=%d (%s)\n", sock->hostname, sock->fd,
          latStr);
    }
    free(latStr);
  }
...
int netasync_resolve(const char *hostname, uint16 port,
...
void netasync_use_socks(struct netasync_socket *sock, const char *hostname,
                        short port) {
  sock->useSocks5 = 1;
  sock->socksPort = port;
...
int netasync_connect(struct netasync_socket *sock,
                     const struct sockaddr_in *addr, int timeout_sec,
                     netasync_callback *cb, void *clientData) {
  int res;
  ...
  if (timeout_sec > 0) {
    ...
    poll_callback_time(netasync.poll, timeout_sec * 1000 * 1000ULL,
                       0 /* !permanent */, netasync_connect_timeout_cb, sock);
  }

  sock->connect_async = 1;
  poll_callback_device(netasync.poll, sock->fd, 0, /* read */
                       1,                          /* writable */
...
static void netasync_fire_errorhandler(struct netasync_socket *sock) {
  ASSERT(sock);
  ASSERT(sock->errorCb);
...
void netasync_set_errorhandler(struct netasync_socket *sock,
                               netasync_callback *callback, void *clientData) {
  ASSERT(sock);
...
static void netasync_receive_cb(void *clientData) {
  ...
  LOG(1, (LGPFX " %s: numRead=%zu (idx=%zu vs len=%zu)\n", __FUNCTION__,
          numRead, sock->recvBufIdx, sock->recvBufLen));
...
int netasync_receive(struct netasync_socket *sock, void *buf, size_t len,
                     bool partial, netasync_recv_callback *cb,
                     void *clientData) {
  ASSERT(sock->magic == SOCK_MAGIC);
...
static void netasync_send_ctx(struct netasync_socket *sock,
...
int netasync_send(struct netasync_socket *sock, const void *buf, size_t len,
                  netasync_callback *callback, void *clientData) {
  struct netasync_send_ctx *ctx;
  bool newSend;
...
static void netasync_free_send_buf(struct netasync_socket *sock) {
...
static void netasync_listen_stop(struct netasync_socket *sock) {
  bool s;

  ASSERT(sock->bind);

  s = poll_callback_device_remove(netasync.poll, sock->fd, 1, /* read */
                                  0,                          /* write */
                                  1,                          /* permanent */
                                  netasync_accept_cb, sock);
  ASSERT(s);
...
void netasync_close(struct netasync_socket *sock) {
  ASSERT(sock->magic == SOCK_MAGIC);

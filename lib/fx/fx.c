...
static void fx_api_parse_json_entry(cJSON *root, const char *name,
                                    struct bitcui_fx *fx_pair, int *i) {
  cJSON *item;
  cJSON *symbol;
  cJSON *last;
  ...
  if (strcmp(item->string, "USD") == 0 || strcmp(item->string, "EUR") == 0 ||
...
static void fx_api_parse_json(void) {
...
static void fx_api_poll_cb(void *clientData) {
  int still_running;
...
static void fx_api_register_poll(void) {
  ...
  if (maxfd == -1) {
    ...
    poll_callback_time(btcui->poll, 100 * 1000 /* 100 msec */,
                       0 /* not permanent */, fx_api_poll_cb, NULL);
    return;
  }
  ...
  for (fd = 0; fd <= maxfd; fd++) {
    ...
    poll_callback_device(btcui->poll, fd, rd, wr, 0 /* not permanent */,
...
static size_t fx_api_curl_write_cb(void *ptr, size_t size, size_t nmemb,
                                   void *userp) {
...
static void fx_periodic_cb(void *clientData) { fx_do_update(); }
...
static void fx_check_unregister_cbs(void) {
  bool s;

...
#define CIRCLIST_CONTAINER(_p, _t, _m) ((_t *)((char *)(_p)-offsetof(_t, _m)))
...
static inline struct circlist_item *circlist_next_item(
    struct circlist_item *p, const struct circlist_item *head) {
  if (head == NULL || p == NULL) {
    return NULL;
  }
  p = p->next;
...
static inline struct circlist_item *circlist_prev_item(
    struct circlist_item *p, const struct circlist_item *head) {
  if (head == NULL || p == NULL) {
    return NULL;
  }
...
static inline bool circlist_empty(const struct circlist_item *list) {
  return list == NULL;
}
...
static inline void circlist_init_item(struct circlist_item *li) {
  li->prev = NULL;
  li->next = NULL;
}
...
static inline void circlist_queue_item(struct circlist_item **list,
...
static inline void circlist_push_item(struct circlist_item **list,
                                      struct circlist_item *item) {
...
static inline void circlist_delete_item(struct circlist_item **list,

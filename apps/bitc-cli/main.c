...
static void bitc_signal_int_default(void) { signal(SIGINT, SIG_DFL); }
...
static struct secure_area *bitc_get_password(void) {
...
static int bitc_encrypt_wallet(struct secure_area *pass_old,
...
static int bitc_add_address(const char *desc, struct secure_area *pass,
                            char **btc_addr) {
  char *errStr = NULL;
  int res;
...
static void bitc_version_and_exit(void) {
  printf(
      "bitc: %s.\n"
      " - compiled on %s at %s,\n"
      " - version: %s.\n",
      BTC_CLIENT_DESC, __DATE__, __TIME__, BTC_CLIENT_VERSION);
...
static void bitc_openssl_exit(void) {
  int i;
...
static int bitc_check_wallet(void) {
  char *btc_addr = NULL;
  char *wltPath;
  int res = 0;
  ...
  printf(
      "Your bitcoin address with this app:\n\n"
      "     %s\n\n",
      btc_addr);
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "a:c:dehn:pt:Tuvz", long_opts, NULL)) !=
         EOF) {
    switch (c) {
      case 'a':
        addr_label = optarg;
  ...
  Log_SetLevel(1);
  {
    ...
    char *logFile;
    logFile = safe_asprintf("/tmp/bitc-%s%s.log", login ? login : "foo",
                            btc->testnet ? "-testnet" : "");
    Log_Init(logFile);
    free(logFile);
    free(login);

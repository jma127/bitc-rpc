 ...
 * Local types.
 */
#define GET_PANEL(li_) CIRCLIST_CONTAINER(li, struct ncpanel, item)
...
static const struct {
  enum PanelType type;
    ...
    {PANEL_DASHBOARD}, {PANEL_WALLET},    {PANEL_CONTACTS}, {PANEL_TX},
    {PANEL_FX},        {PANEL_BLOCKLIST}, {PANEL_PEERS},    {PANEL_LOG},
};
...
static void ncui_input_kbd_cb(struct ncui *ncui, const char *cmd);
...
static uint64 ncui_get_max_tx_amount(void) {
...
static void ncui_get_balance(int64 *confirmed, int64 *unConfirmed,
                             int *numTxUnconf) {
  struct bitcui_tx *tx_info = btcui->tx_info;
  int64 conf = 0;
  int64 pending = 0;
...
static void ncui_doupdate(void) {
...
static void ncui_draw_frame(void) {
  WINDOW *win = stdscr;
...
static void ncui_panel_refresh(const struct ncpanel *panel) {
  ASSERT(mutex_islocked(btcui->lock));
...
static void ncui_panel_check_refresh(const struct ncpanel *panel) {
  struct ncui *ncui = btc->ui;
...
static struct ncpanel *ncui_get_panel_by_type(enum PanelType idx) {
...
static struct ncpanel *ncui_panel_prev(void) {
...
static struct ncpanel *ncui_panel_next(void) {
...
static void ncui_get_term_size(int *y, int *x) {
  struct winsize ws;
...
static void ncui_signal_cb(int sig) {
  int x, y;
...
void ncui_status_update(bool update) {
  ...
  if (type) {
    ...
    mvwprintw(win, LINES - 2,
              COLS - strlen(ncui->timeStr) - 3 - wlen -
                  (proxyStr ? strlen(proxyStr) : 0) - strlen(type),
              "%s", type);
  ...
  if (proxyStr) {
    ...
    mvwprintw(win, LINES - 2,
              COLS - strlen(ncui->timeStr) - 3 - wlen - strlen(proxyStr), "%s",
              proxyStr);
  ...
  mvwprintw(win, LINES - 2, COLS - strlen(ncui->timeStr) - 2, "%s",
            ncui->timeStr);
...
static void ncui_update_time_str(void) {
  struct tm *tmp;
  time_t t;
...
static void ncui_contacts_update(void) {
  ...
  for (i = 0; i < n; i++) {
    ...
    char *label =
        config_getstring(btc->contactsCfg, NULL, "contact%u.label", i);
...
static void ncui_label_top_panel(void) {
  struct circlist_item *li;
  WINDOW *win = stdscr;
  int off;
...
static void ncui_panel_scroll(struct ncpanel *panel, bool down, int delta) {
  int height;
  ...
  Log(LGPFX
      " scroll=%d select=%d numLines=%d/%d select_y=%d scroll_y=%d delta=%d "
      "down=%u\n",
      panel->scroll_on, panel->select_on, panel->num_lines, panel->max_lines,
...
static void ncui_tx_form_destroy(struct ncui *ncui) {
  int i;
...
static void ncui_tx_form_process(struct ncui *ncui) {
...
static void ncui_tx_form_complete(struct ncui *ncui, bool ok) {
  if (ok) {
...
static void ncui_form_input_cb(struct ncui *ncui) {
  bool isCancel;
  bool isOK;
  FIELD *f;
  int c;
  ...
  switch (c) {
    ...
    case 127:
    case KEY_BACKSPACE:
      form_driver(ncui->form, REQ_DEL_PREV);
      break;
    case KEY_DC:
      form_driver(ncui->form, REQ_DEL_CHAR);
      break;
    case KEY_HOME:
      form_driver(ncui->form, REQ_BEG_FIELD);
      break;
    case KEY_END:
      form_driver(ncui->form, REQ_END_FIELD);
      break;
    default:
  ...
  if (ncui->form) {
    ...
    if (isOK || isCancel) {
      ...
      set_field_fore(ncui->field[TX_FIELD_CANCEL],
                     isCancel ? A_REVERSE : A_NORMAL);
...
static void ncui_normal_input_cb(struct ncui *ncui) {
...
static void ncui_destroy_panel(struct ncpanel *panel) {
  if (panel->destroy_cb) {
...
static struct ncpanel *ncui_create_panel(int maxLines, size_t sz) {
  struct ncpanel *panel;
...
static int ncui_ncurses_init(void) {
  int y, x;
  ...
  // --------- ------- ---- -------  ----------- ---------- -----------
  // -----------
  // base03    #002b36  8/4 brblack  234 #1c1c1c 15 -12 -12   0  43  54 193 100
  // 21
  // base02    #073642  0/4 black    235 #262626 20 -12 -12   7  54  66 192  90
  // 26
  // base01    #586e75 10/7 brgreen  240 #585858 45 -07 -07  88 110 117 194  25
  // 46
  // base00    #657b83 11/7 bryellow 241 #626262 50 -07 -07 101 123 131 195  23
  // 51
  // base0     #839496 12/6 brblue   244 #808080 60 -06 -03 131 148 150 186  13
  // 59
  // base1     #93a1a1 14/4 brcyan   245 #8a8a8a 65 -05 -02 147 161 161 180   9
  // 63
  // base2     #eee8d5  7/7 white    254 #e4e4e4 92 -00  10 238 232 213  44  11
  // 93
  // base3     #fdf6e3 15/7 brwhite  230 #ffffd7 97  00  10 253 246 227  44  10
  // 99
  // yellow    #b58900  3/3 yellow   136 #af8700 60  10  65 181 137   0  45 100
  // 71
  // orange    #cb4b16  9/3 brred    166 #d75f00 50  50  55 203  75  22  18  89
  // 80
  // red       #dc322f  1/1 red      160 #d70000 50  65  45 220  50  47   1  79
  // 86
  // magenta   #d33682  5/5 magenta  125 #af005f 50  65 -05 211  54 130 331  74
  // 83
  // violet    #6c71c4 13/5 brmagenta 61 #5f5faf 50  15 -45 108 113 196 237  45
  // 77
  // blue      #268bd2  4/4 blue      33 #0087ff 55 -10 -45  38 139 210 205  82
  // 82
  // cyan      #2aa198  6/6 cyan      37 #00afaf 60 -35 -05  42 161 152 175  74
  // 63
  // green     #859900  2/2 green     64 #5f8700 60 -20  65 133 153   0  68 100
  // 60
...
static void ncui_panel_destroy_all(void) {
...
void ncui_fx_update(void) {
  ...
  for (i = 0; i < btcui->fx_num; i++) {
     ...
     */
    if (strcasecmp(name, "THB") == 0 || strcasecmp(name, "PLN") == 0 ||
        strcasecmp(name, "CNY") == 0 || strcasecmp(name, "JPY") == 0 ||
        strcasecmp(name, "GBP") == 0 || strcasecmp(name, "EUR") == 0) {
      symbol = name;
    }
    wattron(win, PAIR_GREEN);
...
static char *ncui_get_version_str(const char *versionStr) {
  uint32 v0, v1, v2, v3;
  int n;
...
static char *ncui_get_city_str(const struct ipinfo_entry *entry) {
  char cityStr[128];

  if (entry == NULL || entry->city == NULL) {
    return NULL;
  }
  if (strcmp(entry->country_code, "US") == 0 && entry->region_code) {
    snprintf(cityStr, sizeof cityStr, "%s, %s", entry->city,
             entry->region_code);
  } else {
...
static bool ncui_is_pfx_char(char c) {
...
static int ncui_dashboard_color_test(WINDOW *win, int i) {
  int j;
...
static int ncui_dashboard_latest_blocks(WINDOW *win, int y, int num_tx) {
  int last = -1;
  int y0 = y;
  int idx;
  int j;
...
static int ncui_dashboard_peers(WINDOW *win, int y) {
  struct ipinfo_entry *entry;
  size_t host_len = 0;
  size_t ver_len = 0;
  char *v;
...
static int ncui_dashboard_latest_tx(WINDOW *win, int y, int *num_tx) {
  struct bitcui_tx *tx_info = btcui->tx_info;
  time_t now;
  bool header = 0;
  int yLabel = y;
  ...
  *num_tx = n;
  if (n > 0) {
    mvwhline(win, y++, 0, ACS_HLINE, COLS - 1);
    mvwprintw(win, yLabel, 1, "%u transaction%s in the past 48h:", n,
              n > 1 ? "s" : "");
  }
  return y;
}
...
static int ncui_dashboard_balance(WINDOW *win, int y, int len) {
  int numUnconf;
  int64 unconf;
  int64 conf;
  ...
  if (conf == unconf && conf == 0) {
    ...
    mvwprintw(win, y++, 1 + len + 3, " -- num_tx: %u %s", btcui->tx_num,
              numUnconf > 0 ? str : "");
...
static int ncui_dashboard_header(WINDOW *win, int y, size_t *len) {
  uint32 timestamp;
  ...
  mvwprintw(win, y, 1, "peers:   %u / %u", btcui->num_peers_alive,
...
static int ncui_dashboard_sync_info(WINDOW *win, int y) {
  if (bitc_state_ready()) {
    return y;
  }

  if (btcui->numhdr < btcui->hdrtot) {
    mvwprintw(win, y++, 1, "catching-up: headers: %u out of %u", btcui->numhdr,
              btcui->hdrtot);
    mvwhline(win, y++, 0, ACS_HLINE, COLS - 1);
...
static void ncui_panel_init(void) {
  size_t i;
...
static void ncui_on_panic_cb(void *clientData) {
  resetty();
  endwin();
}

...
static bool script_inst_ispush(const struct script_inst *inst) {
...
static bool script_inst_match(const struct script_inst *inst,
...
static void script_check_resize(struct script *script) {
  if (script->len < script->max_len) {
    return;
  }
  script->max_len *= 2;
  script->inst =
      safe_realloc(script->inst, script->max_len * sizeof *script->inst);
}
...
static struct script *script_alloc(void) {
...
static enum script_txout_type script_classify(struct script *script) {
  int i;
...
static int script_push_pubkey(struct wallet *wallet, const uint160 *keyHash,

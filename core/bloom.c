...
static const uint8 bit_mask[8] = {0x01, 0x02, 0x04, 0x08,
                                  0x10, 0x20, 0x40, 0x80};
...
struct bloom_filter *bloom_create(int n, double fp) {
  struct bloom_filter *f;

  f = safe_malloc(sizeof *f);
  f->filterSize =
      MIN((uint32)(-1 / LN2SQUARED * n * log(fp)), MAX_BLOOM_FILTER_SIZE * 8) /
      8;
...
static uint32 bloom_hash(const struct bloom_filter *f, uint32 funIdx,

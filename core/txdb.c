...
static int txdb_remember_tx(struct txdb *txdb, bool alreadySaved,
                            mtime_t timestamp, const uint8 *buf, size_t len,
                            const uint256 *txHash, const uint256 *blkHash,
                            bool *relevant);
...
static void txdb_save_tx_label(const struct btc_tx_desc *tx_desc,
                               const char *hashStr) {
...
static void txdb_print_coins(const struct txdb *txdb, bool onlyUnspent) {
  ...
  for (i = 0; i < n; i++) {
    ...
    Log(LGPFX " txo%03d: %s sp=%u id=%3u v=%llu\n", i, txo_ent->btc_addr,
        txo_ent->spent, txo_ent->outIdx, txo_ent->value);
  }
  free(txo_array);
}
...
static void txdb_free_tx_entry(struct tx_entry *txe) {
...
static void txdb_hashtable_free_tx_entry(const void *key, size_t keyLen,
                                         void *clientData) {
...
static void txdb_hashtable_free_txo_entry(const void *key, size_t keyLen,
                                          void *clientData) {
...
static uint64 txdb_get_tx_credit(const btc_msg_tx *tx) {
  uint64 credit;
  int i;
...
static struct txo_entry *txdb_lookup_txo(const uint256 *txHash, uint32 outIdx) {
...
static uint64 txdb_get_tx_debit(const btc_msg_tx *tx) {
  uint64 debit;
  int i;
...
static struct buff *txdb_serialize_tx_key(uint64 tx_seq, const char *hashStr) {
  struct buff *buf;
  char str[256];
...
static struct tx_ser_key *txdb_deserialize_tx_key(const void *key,
                                                  size_t klen) {
...
static struct tx_ser_data *txdb_deserialize_tx_data(const void *val,
                                                    size_t vlen) {
...
static struct buff *txdb_serialize_tx_data(const struct tx_ser_data *tx) {
  struct buff *buf;
...
static struct tx_entry *txdb_get_tx_entry(const struct txdb *txdb,
                                          const uint256 *hash) {
  struct tx_entry *txe;
  bool s;
...
static char *txdb_get_db_path(struct config *config) {
...
static void txdb_process_tx_entry(struct txdb *txdb, const uint256 *txHash,
                                  const uint256 *blkHash, const btc_msg_tx *tx,
                                  bool *relevant) {
  struct txo_entry *txo_entry;
  char hashStr[80];
  uint32 i;
  ...
  for (i = 0; i < tx->out_count; i++) {
    ...
    if (script_parse_pubkey_hash(txo->scriptPubKey, txo->scriptLength,
                                 &pub_key) ||
...
static void txdb_get_balance_cb(const void *key, size_t klen, void *clientData,
                                void *keyData) {
...
static void txdb_remove_from_hashtable(struct txdb *txdb,
  ...
  Warning(LGPFX " %s removed from hash_tx: %d (count=%u)\n", hashStr, s,
          hashtable_getnumentries(txdb->hash_tx));
}
...
static int txdb_add_to_hashtable(struct txdb *txdb, const void *buf, size_t len,
                                 const uint256 *txHash, const uint256 *blkHash,
                                 uint64 timestamp, struct tx_entry **txePtr) {
  struct tx_entry *txe;
  struct buff b;
  int res;
  bool s;
...
static int txdb_open_db(struct txdb *txdb) {
  char *err = NULL;
...
static int txdb_load_tx(struct txdb *txdb, const char *key, size_t klen,
  ...
  res = txdb_remember_tx(txdb, 1 /* not on disk, just hashtable */,
                         txd->timestamp, txd->buf, txd->len, &txk->txHash,
                         &txd->blkHash, &relevant);
...
int txdb_open(struct config *config, char **errStr, struct txdb **out) {
...
static int txdb_save_tx(struct txdb *txdb, const uint256 *blkHash,
                        const uint256 *txHash, mtime_t timestamp,
  ...
  leveldb_put(txdb->db, txdb->wr_opts, buff_base(bufk), buff_curlen(bufk),
              buff_base(bufd), buff_curlen(bufd), &err);
...
void txdb_confirm_one_tx(struct txdb *txdb, const uint256 *blkHash,
  ...
  while (leveldb_iter_valid(iter)) {
    ...
    leveldb_put(txdb->db, txdb->wr_opts, key, klen, buff_base(buf),
                buff_curlen(buf), &err);
    buff_free(buf);
    if (err) {
...
static int txdb_remember_tx(struct txdb *txdb, bool alreadySaved, mtime_t ts,
                            const uint8 *buf, size_t len, const uint256 *txHash,
                            const uint256 *blkHash, bool *relevant) {
  struct tx_entry *txe;
  char hashStr[80];
  int res;
  ...
  *relevant = 1;
  ...
  if (bitc_state_ready()) {
    ...
    bitcui_set_status("New payment %s: %.8f BTC",
                      value > 0 ? "received" : "made", 1.0 * value / ONE_BTC);
  }
...
int txdb_handle_tx(struct txdb *txdb, const uint256 *blkHash, const uint8 *buf,
                   size_t len, bool *relevant) {
  uint256 txHash;
  mtime_t ts = 0;
  bool txKnown;
  ...
  *relevant = 0;
  ...
  return txdb_remember_tx(txdb, 0 /* save to disk */, ts, buf, len, &txHash,
                          blkHash, relevant);
}
...
static void txdb_set_txo(btc_msg_tx *tx, int idx, const char *btc_addr,
                         uint64 value) {
...
static void txdb_prepare_txout(const struct btc_tx_desc *tx_desc,
                               btc_msg_tx *tx) {
  int i;
...
static void txdb_sign_tx_inputs(struct txdb *txdb, btc_msg_tx *tx) {
  int i;
...
static int txdb_txo_entry_compare_cb(const void *e0, const void *e1) {
...
static struct txo_entry *txdb_get_coins_sorted(struct txdb *txdb) {
...
static void txdb_select_coins(struct txdb *txdb, const struct btc_tx_desc *desc,
                              btc_msg_tx *tx, uint64 *change) {
  ...
  while (value < (desc->total_value + desc->fee) && i < txo_num) {
    ...
    if (txo_ent->spent == 1 || txo_ent->spendable == 0) {
      continue;
    }
    ...
    Log(LGPFX " using txo for %s id=%3u of %s\n", txo_ent->btc_addr,
        txo_ent->outIdx, hashStr);
    value += txo_ent->value;
    memcpy(&tx->tx_in[tx->in_count].prevTxHash, &txo_ent->txHash,
           sizeof txo_ent->txHash);
...
int txdb_craft_tx(struct txdb *txdb, const struct btc_tx_desc *tx_desc,
                  btc_msg_tx *tx) {
  struct buff *buf;
  char hashStr[80];
  uint256 txHash;
  ...
  res = txdb_remember_tx(txdb, 0 /* save to disk */, ts, buff_base(buf),
                         buff_curlen(buf), &txHash, NULL, &relevant);

  txdb_save_tx_label(tx_desc, hashStr);
  txdb_export_tx_info(txdb);

  res = peergroup_new_tx_broadcast(btc->peerGroup, buf, ts + 2 * 60 * 60,
                                   &txHash);
  if (res) {
...
void txdb_close(struct txdb *txdb) {
  if (txdb == NULL) {
    return;
  }
...
static void txdb_export_tx_cb(const void *key, size_t keyLen, void *cbData,
                              void *keyData) {
  ...
  if (!uint256_iszero(&txe->blkHash)) {
    txi->blockHeight =
        blockstore_get_block_height(btc->blockStore, &txe->blkHash);
  }
  ...
  if (txi->desc == NULL) {
    hashStr[63] = '\0';
    txi->desc =
        config_getstring(btc->txLabelsCfg, NULL, "tx.%s.label", hashStr);
  }
...
static int txdb_bitcui_tx_entry_compare_cb(const void *t0, const void *t1) {

...
int wallet_zap_txdb(struct config *config) { return txdb_zap(config); }
...
static void wallet_print_key_cb(const void *key, size_t keyLen, void *cbData,
                                void *keyData) {
...
static void wallet_print(struct wallet *wallet) {
...
static bool wallet_verify_hmac(const struct wallet *wallet, const char *privStr,
                               uint8 **encPrivKey, size_t *encLen) {
  uint8 *hash;
  size_t hash_len;
  uint8 *key;
  size_t key_len;
...
static bool wallet_alloc_key(struct wallet *wallet, const char *priv,
                             const char *pub, const char *desc, time_t birth,
                             bool spendable) {
  struct wallet_key *wkey;
  struct key *key;
  uint160 pub_key;
...
static char *wallet_hmac_string(const uint8 *privkey, size_t privlen,
...
static char *wallet_encrypt_string(struct wallet *wallet, const char *plaintext,
                                   struct crypt_key *ckey) {
...
static void wallet_save_key_cb(const void *key, size_t klen, void *clientData,
                               void *keyData) {
  struct wallet_key *wkey = (struct wallet_key *)keyData;
...
static int wallet_save_keys(struct wallet *wallet) {
  struct config *cfg;
  int res;
  int n;
  ...
  Log(LGPFX " saving %u key%s in %sencrypted wallet %s.\n", n, n > 1 ? "s" : "",
      wallet->pass ? "encrypted" : "NON-", wallet->filename);
  ...
  if (wallet->pass) {
    ...
    str_snprintf_bytes(saltStr, sizeof saltStr, NULL, wallet->ckey->salt,
                       sizeof wallet->ckey->salt);
...
static void wallet_crypt_init(struct wallet *wallet, const char *saltStr,
                              int64 count) {
  int64 count0 = count;
  uint8 *salt;
  size_t len;
...
static int wallet_load_keys(struct wallet *wallet, char **errStr,
                            struct config *cfg,
  ...
  Log(LGPFX " %s wallet: %u key%s in file '%s'.\n",
      *wallet_state == WALLET_PLAIN ? "plain" : "encrypted", n,
      n > 1 ? "s" : "", wallet->filename);
...
static void wallet_update_filter_cb(const void *key, size_t len, void *cbData,
                                    void *keyData) {
...
static void wallet_update_filter(const struct wallet *wallet,
                                 struct bloom_filter *filter) {
...
static void wallet_filter_init(struct wallet *wallet) {
...
struct key *wallet_lookup_pubkey(const struct wallet *wallet,
                                 const uint160 *pub_key) {
  struct wallet_key *wkey;
  bool s;

  s = hashtable_lookup(wallet->hash_keys, pub_key, sizeof *pub_key,
                       (void *)&wkey);
  if (s == 0) {
    return NULL;
  }
...
bool wallet_is_pubkey_spendable(const struct wallet *wallet,
                                const uint160 *pub_key) {
  struct wallet_key *wkey;
  bool s;
...
bool wallet_is_pubkey_hash160_mine(const struct wallet *wallet,
                                   const uint160 *pub_key) {
...
static void wallet_export_addrs_cb(const void *key, size_t len, void *cbData,
                                   void *keyData) {
  struct wallet_key *wkey = (struct wallet_key *)keyData;
...
static int wallet_addr_info_compare(const void *a0, const void *a1) {
...
static void wallet_export_bitcoin_addrs(const struct wallet *wallet,
...
static struct wallet *wallet_open_file(struct config *config,
                                       struct secure_area *pass, char **errStr,
                                       enum wallet_state *wallet_state) {
...
int wallet_open(struct config *config, struct secure_area *pass, char **errStr,
...
void wallet_get_bloom_filter_info(const struct wallet *wallet, uint8 **filter,
                                  uint32 *filterSize, uint32 *numHashFuncs,
                                  uint32 *tweak) {
...
static void wallet_get_birth_cb(const void *key, size_t keyLen,
                                void *clientData, void *keyData) {
  struct wallet_key *wkey = (struct wallet_key *)keyData;
...
static void wallet_free_key_cb(const void *key, size_t keyLen,
                               void *clientData) {
...
int wallet_craft_tx(struct wallet *wallet, const struct btc_tx_desc *desc,
                    btc_msg_tx *tx) {
  uint64 value = 0;
  int i;
  ...
  if (value + desc->fee > wallet->balance) {
    Warning(LGPFX " insufficient funds: %llu vs %llu (%.8f vs %.8f) fee=%.8f\n",
            value, wallet->balance, value / ONE_BTC, wallet->balance / ONE_BTC,
            desc->fee / ONE_BTC);
    return 1;
  }
...
static void wallet_find_key_by_idx_cb(const void *key, size_t klen,
                                      void *clientData, void *keyData) {
...
static struct wallet_key *wallet_find_key_by_idx(struct wallet *wallet,
                                                 uint32 cfg_idx) {
...
void wallet_confirm_tx_in_block(struct wallet *wallet,
...
static const char *wallet_state_to_str(void) {
...
bool wallet_verify(struct secure_area *passphrase,

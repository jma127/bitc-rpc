...
static void base58_getvch(const BIGNUM *v, uint8 **buf, size_t *len) {
  uint32 size;
  uint8 *s;
...
static void base58_decode(const char *str, uint8 **dst, size_t *dstLen) {
  const char *buf;
  BN_CTX *ctx;
  BIGNUM bn58;
  BIGNUM bnc;
  ...
  *dstLen = 0;
  ...
  if (len >= 2 && tmp[len - 1] == 0 && ((uint8)tmp[len - 2] >= 0x80)) {
    len--;
  }
...
static bool base58_decode_check(uint8 *addrtype, const char *buf,
                                uint8 **dataOut, size_t *lenOut) {
  uint8 *data;
  size_t len;
  uint8 h[4];
...
static void base58_setvch(BIGNUM *bn, const void *buf0, size_t len0) {
  uint32 len = len0 + 4;
  uint8 buffer[len];
  BIGNUM *res;
...
static char *base58_encode(const void *buf, size_t len) {
...
static char *base58_encode_check(uint8 addrtype, bool have_addrtype,
...
bool b58_privkey_to_bytes(const char *addr, uint8 **key, size_t *len) {
  uint8 *buf = NULL;
  uint8 type;

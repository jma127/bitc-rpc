...
void btcmsg_print_header(const btc_block_header *header)  // IN
{
  char prevStr[128];
  char mrklStr[128];
  char *timeStr;
...
void btcmsg_print_txout(const btc_msg_tx_out *txOut) {
  ...
  if (slen == 25 && txOut->scriptPubKey && txOut->scriptPubKey[0] == OP_DUP &&
    ...
    str_snprintf_bytes(str, sizeof str, NULL, txOut->scriptPubKey + 3,
                       txOut->scriptLength - 5);
    snprintf(scriptStr, sizeof scriptStr, "DUP HASH160 %s EQUALVERIFY CHECKSIG",
             str);
  } else {
    str_snprintf_bytes(scriptStr, sizeof scriptStr, NULL, txOut->scriptPubKey,
                       txOut->scriptLength);
  }
...
void btcmsg_print_txin(const btc_msg_tx_in *txIn) {
  ...
  str_snprintf_bytes(scriptStr, sizeof scriptStr, NULL, txIn->scriptSig,
                     txIn->scriptLength);
...
static inline bool btcmsg_addr_is_ipv4(const struct btc_msg_address *addr) {
...
bool btcmsg_payload_valid(const struct buff *buf, const uint8 checksum[4]) {
  uint8 cksum[4] = {0};
...
static bool bit_isset(const uint8 *bitArray, uint32 i) {
  ASSERT(bitArray);
...
static uint32 btcmsg_get_width(const btc_msg_merkleblock *blk, uint32 height) {
...
static uint256 btcmsg_walk_tree(btc_msg_merkleblock *blk, uint32 height,
                                uint32 pos, uint32 *bitIdx, uint32 *hashIdx) {
  uint256 hash;
  bool parent;
...
static bool btcmsg_verify_merkle_tree(btc_msg_merkleblock *blk) {
  uint256 root;
  uint32 bitIdx = 0;
  uint32 hashIdx = 0;
  int height = 0;
...
int btcmsg_parse_merkleblock(struct buff *buf, btc_msg_merkleblock **blkOut) {
  ...
  *blkOut = NULL;
  ...
  LOG(0, (LGPFX " txCount=%u hashCount=%llu bitArraySz=%llu\n", blk->txCount,
          blk->hashCount, blk->bitArraySize));
  if (res) {
    goto error;
  }
...
int btcmsg_parse_headers(struct buff *buf, btc_block_header **headersOut,
                         int *num) {
...
static int btcmsg_craft_msgheader(struct buff **bufOut, const char *message,
                                  struct buff *bufData) {
  struct buff *buf;
  btc_msg_header h;
...
int btcmsg_craft_filterload(const btc_msg_filterload *fl,
                            struct buff **bufOut) {
  struct buff *buf;
...
static int btcmsg_prepare_version(struct buff *buf) {
  btc_msg_version v;
  int res;
...
static struct btc_block_locator *btcmsg_prepare_blocklocator(
    const uint256 *hashes, int num, const uint256 *stop) {
...
int btcmsg_craft_getblocks(const uint256 *hashes, int num,
                           struct buff **bufOut) {
...
int btcmsg_craft_getheaders(const uint256 *hashes, int num,
                            const uint256 *genesis, struct buff **bufOut) {
  btc_block_locator *bl;
  struct buff *buf;
...
int btcmsg_craft_verack(struct buff **bufOut) {
...
int btcmsg_craft_mempool(struct buff **bufOut) {
...
int btcmsg_craft_getaddr(struct buff **bufOut) {
...
int btcmsg_craft_version(struct buff **bufOut) {
  struct buff *buf;
...
void btcmsg_print_version(const char *pfx, const btc_msg_version *v) {
  ...
  Log(LGPFX " %s: '%s' @%d -- '%s' -- height=%u --svc=%#llx\n", pfx,
      v->strVersion, v->version, s, v->startingHeight, v->services);
...
static void btcmsg_print_addr(const btc_msg_address *addr, const char *pfx) {
  char *s;

  s = print_time_utc(addr->time);
  Log(LGPFX " %s : p=%u %s -- ip: %u.%u.%u.%u\n", pfx, ntohs(addr->port), s,
      addr->ip[12 + 0], addr->ip[12 + 1], addr->ip[12 + 2], addr->ip[12 + 3]);
  free(s);
}
...
int btcmsg_craft_addr(uint32 protversion, const struct btc_msg_address *addrs,
                      size_t numAddrs, struct buff **bufOut) {
  struct buff *buf;
  size_t i;
...
int btcmsg_parse_addr(uint32 protversion, struct buff *buf,
                      struct btc_msg_address ***addrsOut, size_t *numAddrsOut) {
...
void btc_msg_tx_init(btc_msg_tx *tx) { memset(tx, 0, sizeof *tx); }

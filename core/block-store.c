...
void blockstore_get_highest(struct blockstore *bs, const uint256 *hash0,
                            const uint256 *hash1, uint256 *hash) {
  int height0;
  int height1;
...
static bool blockstore_validate_chkpt(const uint256 *hash, uint32 height) {
...
static void blockstore_set_best_chain(struct blockstore *bs,
                                      struct blockentry *be,
                                      const uint256 *hash) {
  int height;
...
bool blockstore_is_block_known(const struct blockstore *bs,
                               const uint256 *hash) {
  return blockstore_has_header(bs, hash) || blockstore_is_orphan(bs, hash);
}
...
static struct blockentry *blockstore_alloc_entry(const btc_block_header *hdr) {
  struct blockentry *be;
...
void blockstore_write_headers(struct blockstore *bs) {
  ...
  res = file_pwrite(bs->blockSet->desc, bs->blockSet->filesize, buf,
                    numhdr * sizeof *buf, &numWritten);
  free(buf);
...
static void blockset_close(struct blockset *bset) {
...
static int blockset_open(struct blockstore *blockStore, const char *filename) {
  struct blockset *bs;
...
void blockstore_get_hash_from_birth(const struct blockstore *bs, uint64 birth,
                                    uint256 *hash) {
  struct blockentry *e;
...
bool blockstore_is_next(struct blockstore *bs, const uint256 *prev,
                        const uint256 *next) {
  struct blockentry *be;
  uint256 hash;
  bool s;
...
void blockstore_get_locator_hashes(const struct blockstore *bs, uint256 **hash,
                                   int *num) {
...
void blockstore_get_genesis(const struct blockstore *bs, uint256 *genesis) {

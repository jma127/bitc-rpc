...
static int crypt_determine_count(const struct secure_area *pass,
  ...
  while (loop > 0) {
    ...
    ts = time_get();
    len = EVP_BytesToKey(EVP_aes_256_cbc(), EVP_sha512(), ckey->salt, pass->buf,
                         pass->len, count, ckey->key, ckey->iv);
...
bool crypt_set_key_from_passphrase(const struct secure_area *pass,
                                   struct crypt_key *ckey, int64 *count_ptr) {
  int count;
  int len;
...
bool crypt_encrypt(struct crypt_key *ckey, const struct secure_area *plaintext,
  ...
  *cipher_len = 0;
  ...
  res =
      res && EVP_EncryptUpdate(&ctx, c, &clen, plaintext->buf, plaintext->len);
...
bool crypt_decrypt(struct crypt_key *ckey, const uint8 *cipher,
                   size_t cipher_len, struct secure_area **plaintext) {
...
void crypt_hmac_sha256(const void *text, size_t text_len, const uint8 *key,
                       size_t key_len, uint256 *digest) {
  uint8 buffer[1024];
  uint256 key_hash;
  uint256 buf_hash;
  uint8 ipad[65];

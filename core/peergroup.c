...
static void peergroup_free_tx_broadcast_entry(struct tx_broadcast *txb) {
  buff_free(txb->buf);
  free(txb);
}
...
static void peergroup_free_tx_broadcast_cb(const void *key, size_t keylen,
                                           void *clientData) {
...
static void peergroup_set_lastblk(struct peergroup *pg, const uint256 *hash) {
  char prev[80];
  char next[80];
...
static void peergroup_get_lastblk(const struct peergroup *pg, uint256 *hash) {
  ASSERT(pg);
...
static uint32 peergroup_count_connected(void) {
...
static void peergroup_update_info(void) {
  struct peergroup *pg = btc->peerGroup;
  struct circlist_item *li;
...
static void peergroup_add_peer(struct peer_addr *paddr) {
  static int last;

  btc->peerGroup->active++;
  if ((btc->peerGroup->active % 250) == 0 && btc->peerGroup->active != last) {
...
static void peergroup_download_progress(void) {
...
static void peergroup_on_ready(void) {
  struct circlist_item *li;

  Log(LGPFX " peergroup ready.\n");
  bitcui_set_status("online.");

  CIRCLIST_SCAN(li, btc->peerGroup->peer_list) { peer_on_ready_li(li); }
}
...
static void peergroup_download_complete(void) {
...
static void peergroup_add_block_finalize(struct blockstore *bs, bool headerOnly)
...
static void peergroup_process_filtered_block(struct peer *peer,
...
int peergroup_download_headers(struct peer *peer, int peerStartingHeight) {
  struct blockstore *bs = btc->blockStore;
  ...
  if (peerStartingHeight > btc->peerGroup->heightTarget) {
    if (btc->peerGroup->numHdrToFetch == 0) {
      btc->peerGroup->numHdrToFetch =
          peerStartingHeight - blockstore_get_height(bs);
    } else {
      btc->peerGroup->numHdrToFetch +=
          peerStartingHeight - btc->peerGroup->heightTarget;
    }
  ...
  if (btc->state == BITC_STATE_STARTING) {
    ...
    if (btc->peerGroup->numHdrToFetch > 0) {
      ...
      Warning(LGPFX " downloading %d header%s -- %s late\n",
              btc->peerGroup->numHdrToFetch,
              btc->peerGroup->numHdrToFetch > 1 ? "s" : "", lagStr);
      free(lagStr);
    }
  }
...
int peergroup_download_filtered_blocks(struct peer *peer) {
  struct blockstore *bs = btc->blockStore;
  uint256 walletHash;
  ...
  if (first) {
    ...
    btc->peerGroup->numToFetch =
        blockstore_get_height(bs) - blockstore_get_block_height(bs, &startHash);
...
int peergroup_download_filtered_blocks_continue(struct peer *peer) {
  uint256 best_hash;
  uint256 lastTxdb;
  int res = 0;
...
static void peergroup_print_stats(struct peergroup *peerGroup) {
  enum btc_msg_type i;
  ...
  for (i = 0; i < BTC_MSG_MAX; i++) {
    if (cmdStats[i].received != 0 || cmdStats[i].sent != 0) {
      Log(LGPFX " %11s: %6u  / %5u\n", btcmsg_type_to_str(i),
          cmdStats[i].received, cmdStats[i].sent);
    }
  }
}
...
static void peergroup_check_liveness(void) {
...
static void peergroup_periodic_cb(void *clientData) {
  if (bitc_exiting()) {
    return;
  }
  peergroup_refill(FALSE);
...
void peergroup_init(struct config *config, uint32 maxPeers, uint32 minPeersInit,
                    mtime_t periodUsec) {
  struct peergroup *pg;
  char *hashStr;
...
static void peergroup_add_peer_from_str(struct poll_loop *poll,
...
static void peergroup_destroy_peers(void) {
...
void peergroup_exit(struct peergroup *pg) {
  ...
  hashtable_clear_with_callback(pg->hash_broadcast,
                                peergroup_free_tx_broadcast_cb);
...
static int peergroup_broadcast_inv(struct peergroup *pg, struct buff *bufInv) {
  struct circlist_item *next;
  struct circlist_item *li;
  int res = 0;
...
static void peergroup_add_tx_broadcast_hash(struct peergroup *pg,
...
static int peergroup_tx_broadcast(struct peergroup *pg, const uint256 *hash) {
  struct buff *bufInv;
  int res;
...
int peergroup_handle_merkleblock(struct peer *peer,
                                 const btc_msg_merkleblock *blk) {

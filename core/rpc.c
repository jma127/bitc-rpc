...
static int http_body_handler(http_parser *parser, const char *at,
                             size_t length) {
...
static int http_on_headers_complete_handler(http_parser *parser) {
...
static struct rpc_client_data *rpc_client_data_alloc(void) {
  struct rpc_client_data *data;
  ...
  data->parser_settings->on_headers_complete = http_on_headers_complete_handler;
  data->offset = -1;
...
static void rpc_client_data_free(struct rpc_client_data *data) {
  ASSERT(data);
...
static void rpc_send_cb(struct netasync_socket *socket, void *data, int err) {
...
static void rpc_send_error_cb(struct netasync_socket *socket, void *data,
                              int err) {
...
static void rpc_error_cb(struct netasync_socket *socket, void *data, int err) {
 ...
 *-------------------------------------------------------------------------
 */
static void send_response(struct netasync_socket *socket, int http_status,
                          json_t *result, json_t *error, json_t *id) {
...
exit:
  ...
  netasync_send(socket, buff_base(resp_str), buff_curlen(resp_str), rpc_send_cb,
                resp_str);
...
static void rpc_process_request(struct netasync_socket *socket,
                                struct rpc_client_data *data) {
  ...
  if (method_data == NULL || params_data == NULL ||
    ...
    Log(LGPFX
        " %s:%u No method (string) or parameters (array) "
        "provided in request\n",
        __FUNCTION__, __LINE__);
    resp_error = json_object();
  ...
  if (method_handler == NULL) {
    ...
    resp_error = json_object();
    json_object_set_new(resp_error, "name", json_string("InvalidMethod"));
    goto exit;
  }
...
static void rpc_receive_cb(struct netasync_socket *socket, void *start,
                           size_t len, void *cdata) {
  struct rpc_client_data *data;
  size_t nparsed;
  ...
  nparsed =
      http_parser_execute(data->parser, data->parser_settings, start, len);
  if (data->parser->upgrade) {
    ...
    Log(LGPFX " %s:%u -- error parsing HTTP request: %d (%s: %s)\n",
        __FUNCTION__, __LINE__, data->parser->http_errno,
...
static void rpc_accept_cb(struct netasync_socket *socket, void *_data,
                          int err) {
  struct rpc_client_data *data;
